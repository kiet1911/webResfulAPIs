...
namespace webResfulAPIs.Helpers.Tokens
{
    public static class GenerateTokens
    {
        ...
        public static string GenerateSnapshotOrder(IConfiguration configuration  , List<CartItemList.CartItemSnapshot> cartItemSnapshots)
        {
            var cartStringJson = System.Text.Json.JsonSerializer.Serialize(cartItemSnapshots);

            try
            {
                string secretkey = configuration["Secret:keys"] ?? throw new Exception("secret key not found");
                var symmetric = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(secretkey));
                var sign = new SigningCredentials(symmetric, SecurityAlgorithms.HmacSha256);
                var descriptor = new SecurityTokenDescriptor
                {
                    Subject = new ClaimsIdentity([
                        new Claim("cartSnapshot",cartStringJson)
                        ]),
